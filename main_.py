...
def main_train_wrapper(
    ...
    random.seed(seed)                     # Seeds Python's built-in random module (e.g. for random.shuffle)
    np.random.seed(seed)                  # Seeds NumPy's random number generator
    torch.manual_seed(seed)               # Seeds PyTorch CPU and current GPU RNG (for cuda:0)
    if device == "cuda":
        torch.cuda.manual_seed(seed)      # Explicitly seeds the current GPU (optional if manual_seed already called)

    torch.backends.cudnn.deterministic = True  # Uses deterministic convolution algorithms in cuDNN
    torch.backends.cudnn.benchmark = False     # Disables non-deterministic auto-tuner for convolutions
    # torch.use_deterministic_algorithms(True)   # Forces PyTorch to use only deterministic ops (errors if not possible)
    print(device)
