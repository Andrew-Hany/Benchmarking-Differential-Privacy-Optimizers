...
class DPOptimizer_Matrix(DPOptimizer):
    def __init__(
        ...
        self.sens_C = sens_C
        self.steps_per_epoch = kwargs['optimizer_steps_per_epoch']
        self.num_epochs = kwargs['epochs']
        self.num_steps =  self.steps_per_epoch*self.num_epochs
        ...
        self.Z_correlated_file = None
        self.lamda_matrix = lamda_matrix
        self.max_grad_norm = 1 if self.normalize_clipping else self.max_grad_norm
    ...
    def _generate_all_noise(self):
        ...
        B_file = "B_matrix.npy"
        Z_file = f"Z_std={self.noise_multiplier * self.max_grad_norm}_{T}_{D}.npy"
        if self.lamda_matrix == True:
            self.Z_correlated_file = f"correlated_Z_std={self.noise_multiplier * self.max_grad_norm}_{self.num_epochs}x{self.steps_per_epoch}_{D}Lambda.raw"
        else:
            self.Z_correlated_file = f"correlated_Z_std={self.noise_multiplier * self.max_grad_norm}_{self.num_epochs}x{self.steps_per_epoch}_{D}.raw"

        if os.path.exists(Z_file):
            print(f"[SKIP] File already exists: {Z_file}")
            print('Warning: if you changed the seed, you need to regenerate the file')
        else:
        ...
        if os.path.exists(self.Z_correlated_file ):
            print(f"[SKIP] File already exists: {self.Z_correlated_file }")
            print('Warning: if you changed the seed, you might need to regenerate the file')
        else:
            gpu_matrix_multiply(
                T=T,
                D=D,
                a_path=self.B_path,
                b_path=Z_file,
                out_path=self.Z_correlated_file ,
                # tile_size=1024
            )
    ...
    def add_noise(self):
        ...
        step_noise_correlated_flat_np = self.get_correlated_step(self.current_step,T,D,memmap_path=self.Z_correlated_file )
